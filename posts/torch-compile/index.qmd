...
![Dynamo captures the FX graph; Inductor lowers it to Triton/C++](images/dynamo-inductor-pipeline.png)
...
![Default CPython frame evaluation (left) vs TorchDynamo's patched frame evaluation with guards and FX graph capture (right)](images/dynamo-cpython-frame-eval.png)
...
![Graph breaks split the Python function into compiled regions separated by eager interpreter calls; the modified bytecode shows the COMPILED REGION / GRAPH BREAK / RESUME POINT structure](images/graph-break-bytecode.png)
...
![torch.export: principles of graph capture — safety vs. coverage trade-off between `torch.export` (strict, static) and `torch.trace` (lenient, dynamic)](images/export-graph-capture-principles.png)

![torch.export container specialization: list/dict inputs get burned into the graph; `torch.export.register_dataclass` enables dataclass inputs](images/export-container-specialization.png)
